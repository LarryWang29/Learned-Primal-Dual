...
def train_network(input_dimension=362, n_detectors=543,
    ...
    if not resume:
        f = open(f"./checkpoints/UNet_checkpoints_{option}/validation_metrics.csv", "w")
        f.write("Epoch, MSE_avg, MSE_std, PSNR_avg, PSNR_std, SSIM_avg, SSIM_std\n")
    else:
        f = open(f"./checkpoints/UNet_checkpoints_{option}/validation_metrics.csv", "a")
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 5 == 0:
            utils.save_checkpoint(epoch, model, optimizer, scheduler, loss, 
                            f"./checkpoints/UNet_checkpoints_{option}/checkpoint_epoch{epoch+1}.pt")
