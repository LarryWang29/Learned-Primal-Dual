"""
This script is used to train the leanred Primal Dual Hybrid Gradient network, which was one of the alternative
...
def train_network(input_dimension=362, n_detectors=543,
    ...
    """
    This function trains the continuous Learned Primal-Dual Hybrid Gradient network on the training dataset.
    ...
    if not resume:
        f = open("./checkpoints/learned_PDHG_checkpoints/validation_metrics.csv", "w")
        f.write("Epoch, MSE_avg, MSE_std, PSNR_avg, PSNR_std, SSIM_avg, SSIM_std\n")
    else:
        f = open("./checkpoints/learned_PDHG_checkpoints/validation_metrics.csv", "a")
    ...
    for epoch in range(epochs):
        ...
        utils.save_checkpoint(epoch, model, optimizer, scheduler, loss, 
                        f"./checkpoints/learned_PDHG_checkpoints/checkpoint_epoch{epoch+1}.pt")
