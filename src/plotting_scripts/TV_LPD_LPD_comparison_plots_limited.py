"""
This script is used to generate figures to compare the performances of LPD and TV-LPD under limited
view scenarios by visualising their reconstructions zoomed in on particular subsections, as well as 
the difference images. Specifically, figures 4.7 and 4.8 in the thesis document are generated using
this script.
"""
...
if __name__ == "__main__":
    ...
    dicts = torch.load("./checkpoints/tv_checkpoints_limited/checkpoint_epoch50.pt")
    ...
    dicts = torch.load("./checkpoints/LPD_checkpoints_limited/checkpoint_epoch50.pt")
    ...
    for test_data in tqdm(test_dataloader):
        data_range = np.max(test_data[1].cpu().numpy()) - np.min(
            test_data[1].cpu().numpy()
        )
        ...
        if (
            tv_model_mse > model_mse
            or tv_model_psnr < model_psnr
            or tv_model_ssim < model_ssim
        ):
            counter += 1
    ...
    ax1.set_yticks([])
    ax1.imshow(
        ground_truth.detach().cpu().numpy().squeeze(0), vmin=0, vmax=1, cmap="gray"
    )
    ...
    # Plot for TV-LPD difference image
    ...
    # Make a plot to show inconsistent geometry being created
    plt.figure(figsize=(10, 5))
    ax1 = plt.subplot(1, 2, 1)
    # Hide ticks
    ax1.set_xticks([])
    ax1.set_yticks([])
    ax1.imshow(tv_output.detach().cpu().numpy().squeeze(0), vmin=0, vmax=1, cmap="gray")
    ax1.set_title("Reconstruction from learned model")
    add_zoomed_inset(
        ax1,
        tv_output.detach().cpu().numpy().squeeze(0),
        [325, 120],
    ...
    plt.tight_layout(rect=[0, 0.05, 1, 0.95])
    plt.savefig("figures/inconsistent_geometry/inconsistent_geometry.png")
